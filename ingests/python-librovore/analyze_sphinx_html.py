    ...
    for selector in code_selectors:
        ...
        for block in blocks:
            block_info = {
                ...
                'parent_classes': list(block.parent.get('class', [])) if block.parent else [],
            ...
            for cls in all_classes:
                if any(lang in cls.lower() for lang in ['python', 'javascript', 'c++', 'java', 'html', 'css', 'bash']):
...
def analyze_api_documentation(soup, theme_name):
    ...
    for dl in dls:
        ...
        for dt in dt_elements:
            ...
            dt_text = dt.get_text()
            if any(keyword in dt_text for keyword in ['def ', 'function', 'class ', 'async ', '(', ')']):
                results['function_signatures'].append(signature_info)

            results['dt_dd_patterns'].append(signature_info)

            # Collect CSS classes
            for cls in signature_info['dt_classes'] + signature_info['dd_classes']:
...
def analyze_section_structure(soup, theme_name):
    ...
    section_selectors = ['section', '.section', '.content', '.body', 'article', 'main']
    for selector in section_selectors:
        ...
        for section in sections:
            section_info = {
                ...
                'child_headings': len(section.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6'])),
...
def analyze_html_file(file_path):
    ...
    theme_name = file_path.stem.split('-')[0]  # Extract theme from filename
...
def main():
    ...
    for analysis in all_analyses:
        ...
        print(f"{theme}: {code_blocks} code blocks, {syntax_patterns} syntax patterns, "
